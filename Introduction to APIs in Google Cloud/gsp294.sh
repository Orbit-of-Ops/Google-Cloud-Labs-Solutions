...
echo "${BOLD}${YELLOW}[Orbit of Ops] Step 1: Setting environment variables...${RESET}"
export OAUTH2_TOKEN=$(gcloud auth print-access-token)
export USERNAME=$(gcloud config get-value core/account | sed 's/-/_/g; s/@.*//')
echo "Username configured as: $USERNAME"
echo ""

# Step 2: Enable Google Fitness API
echo "${BOLD}${BLUE}[Orbit of Ops] Step 2: Enabling Google Fitness API...${RESET}"
gcloud services enable fitness.googleapis.com
echo ""

# Step 3: Create JSON configuration for Google Cloud Storage bucket
echo "${BOLD}${CYAN}[Orbit of Ops] Step 3: Creating bucket configuration (values.json)...${RESET}"
cat > values.json <<EOF
{
...
echo "${BOLD}${MAGENTA}[Orbit of Ops] Step 4: Creating Google Cloud Storage bucket...${RESET}"
...
echo "${BOLD}${YELLOW}[Orbit of Ops] Step 5: Downloading demo image...${RESET}"
curl -LO https://raw.githubusercontent.com/Orbit-of-Ops/Labs-Solutions/refs/heads/main/Introduction%20to%20APIs%20in%20Google%20Cloud/demo-image.png
echo ""

# Step 6: Upload demo image to the bucket
echo "${BOLD}${GREEN}[Orbit of Ops] Step 6: Uploading demo image to the bucket...${RESET}"
