...
function check_progress {
    while true; do
        echo
        echo -n "${BOLD}${YELLOW}[Orbit of Ops] Have you checked your progress up to Task 4? (Y/N): ${RESET}"
        read -r user_input
        if [[ "$user_input" == "Y" || "$user_input" == "y" ]]; then
            ...
            echo
            echo "${BOLD}${RED}Please check your progress up to Task 4 in the lab interface, then press Y to continue.${RESET}"
        else
            echo
...
echo "${BOLD}${RED}[Orbit of Ops] Step 1: Creating bucket1.json...${RESET}"
...
echo "${BOLD}${GREEN}[Orbit of Ops] Step 2: Creating bucket1 via API...${RESET}"
curl -X POST \
...
echo "${BOLD}${YELLOW}[Orbit of Ops] Step 3: Creating bucket2.json...${RESET}"
...
echo "${BOLD}${BLUE}[Orbit of Ops] Step 4: Creating bucket2 via API...${RESET}"
curl -X POST \
  -H "Authorization: Bearer $(gcloud auth print-access-token)" \
  -H "Content-Type: application/json" \
...
echo "${BOLD}${MAGENTA}[Orbit of Ops] Step 5: Downloading the image file...${RESET}"
curl -LO https://raw.githubusercontent.com/Orbit-of-Ops/Labs-Solutions/refs/heads/main/Use%20APIs%20to%20Work%20with%20Cloud%20Storage%20Challenge%20Lab/world.jpeg
echo ""

# Step 6: Upload image file to bucket1
echo "${BOLD}${CYAN}[Orbit of Ops] Step 6: Uploading the image file to bucket1...${RESET}"
curl -X POST \
  -H "Authorization: Bearer $(gcloud auth print-access-token)" \
...
echo "${BOLD}${RED}[Orbit of Ops] Step 7: Copying the image from bucket1 to bucket2...${RESET}"
curl -X POST \
  -H "Authorization: Bearer $(gcloud auth print-access-token)" \
  -H "Content-Type: application/json" \
...
echo "${BOLD}${GREEN}[Orbit of Ops] Step 8: Setting public access for the image...${RESET}"
...
echo "${BOLD}${CYAN}[Orbit of Ops] Step 9: Deleting the image from bucket1...${RESET}"
curl -X DELETE \
  -H "Authorization: Bearer $(gcloud auth print-access-token)" \
...
echo "${BOLD}${BLUE}[Orbit of Ops] Step 10: Deleting bucket1...${RESET}"
curl -X DELETE \
  -H "Authorization: Bearer $(gcloud auth print-access-token)" \
